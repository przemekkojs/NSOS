...
test.describe("Admin Course Management", () => {
  ...
  test.describe("Course Creation", () => {
    ...
    test("Form validation - negative values", async ({ page }) => {
      ...
      await expect(
        page.getByText(/must be positive|greater than 0/i).first()
      ).toBeVisible();
    });
  ...
  test.describe("Course Viewing", () => {
    ...
    test("Non-existent course shows error", async ({ page }) => {
      ...
      await expect(
        page.getByText(/not found|course does not exist|404/i).first()
      ).toBeVisible({
        timeout: 10000,
      });
    });
  ...
  test.describe("Course Editing", () => {
    ...
    test("Admin can update course information", async ({ page }) => {
      ...
      // Verify success notification
      await expect(
        page.getByText(/updated successfully|success/i)
      ).toBeVisible();
  ...
  test.describe("Course Deletion", () => {
    test("Admin can delete a course", async ({ page }) => {
      ...
      // Verify success notification
      await expect(
        page.getByText(/deleted successfully|removed/i)
      ).toBeVisible();
  ...
  test.describe("Error Handling", () => {
    ...
    test("Handles network timeout gracefully", async ({ page }) => {
      ...
      await expect(
        page
          .getByText(/timeout|taking too long|error/i)
