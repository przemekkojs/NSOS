...
function storagePlugin(context: PiniaPluginContext) {
  if (context.options.storage === undefined) return;

  const storage =
    context.options.storage === "localStorage" ? localStorage : sessionStorage;
  ...
  if (storedState) {
    try {
      ...
      console.error(
        `Failed to parse stored state fro ${context.store.$id}: `,
        error
      );
    }
  }
