...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING("Transaction rolled back - no changes were made.")
            )
            raise
    ...
    def create_classes(self):
        ...
        for course_group in course_groups:
            ...
            for week in range(1, 4):
                ...
                if created:
                    self.stdout.write(
                        f"  ✓ Created class: {course_group.course.name} on {date_held}"
                    )
    ...
    def create_schedules(self):
        ...
        for week in range(4):
            ...
            for course_group in course_groups:
                ...
                if created:
                    self.stdout.write(
                        f"  ✓ Created lecturer schedule: {course_group.course.name} on {date}"
                    )
                ...
                for student in students:
                    if student.faculty == course_group.course.faculty:
                        ...
                        if created and week == 0:  # Only log first week to avoid spam
                            self.stdout.write(
                                f"  ✓ Created student schedule: {student.user.username} - {course_group.course.name}"
                            )
