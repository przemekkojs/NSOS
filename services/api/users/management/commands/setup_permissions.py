...
from university.models import University, Faculty, Position, Semester, UniversityMembership
...
class Command(BaseCommand):
    ...
    def create_faculties(self):
        ...
        faculties = [
            {'name': 'Faculty of Computer Science', 'description': 'Computer Science and Information Technology'},
            {'name': 'Faculty of Engineering', 'description': 'Mechanical and Civil Engineering'},
            {'name': 'Faculty of Mathematics', 'description': 'Pure and Applied Mathematics'},
            {'name': 'Faculty of Physics', 'description': 'Theoretical and Experimental Physics'},
            {'name': 'Faculty of Business', 'description': 'Business Administration and Management'},
        ]
    ...
    def create_positions(self):
        ...
        positions = [
            ...
            {'name': 'Associate Professor', 'hourly_rate': 120.00, 'workload': Position.FULL},
            {'name': 'Assistant Professor', 'hourly_rate': 100.00, 'workload': Position.FULL},
            {'name': 'Lecturer', 'hourly_rate': 80.00, 'workload': Position.FULL},
            {'name': 'Teaching Assistant', 'hourly_rate': 60.00, 'workload': Position.HALF},
            {'name': 'Adjunct Professor', 'hourly_rate': 90.00, 'workload': Position.THREE_QUARTER},
        ]
    ...
    def create_students(self):
        ...
        for data in students_data:
            ...
            if created:
                self.stdout.write(f'  ✓ Created student: {user.get_full_name()} ({data["index_number"]})')
    ...
    def create_university_memberships(self):
        ...
        for lecturer in lecturers:
            ...
            if created:
                self.stdout.write(f'  ✓ Created membership: {lecturer.user.get_full_name()} @ {university.name}')
    ...
    def create_course_groups(self):
        ...
        for idx, course in enumerate(courses):
            ...
            for group_num in range(1, 3):
                ...
                if created:
                    self.stdout.write(f'  ✓ Created: {course.course_code} - {course_group.name}')

    def create_classes(self):
        self.stdout.write('\nCreating classes...')
        
        course_groups = CourseGroup.objects.all()[:5]  # First 5 course groups
        
        for course_group in course_groups:
            # Create 3 past classes
            for week in range(1, 4):
                date_held = timezone.now().date() - timedelta(weeks=week)
                ...
                if created:
                    self.stdout.write(f'  ✓ Created class: {course_group.course.name} on {date_held}')

    def create_schedules(self):
        self.stdout.write('\nCreating schedules...')
        
        course_groups = list(CourseGroup.objects.all()[:4])
        ...
        for week in range(4):
            ...
            for course_group in course_groups:
                ...
                if created:
                    self.stdout.write(f'  ✓ Created lecturer schedule: {course_group.course.name} on {date}')
                
                # Schedule for students in the same faculty
                for student in students:
                    if student.faculty == course_group.course.faculty:
                        ...
                        if created and week == 0:  # Only log first week to avoid spam
                            self.stdout.write(f'  ✓ Created student schedule: {student.user.username} - {course_group.course.name}')